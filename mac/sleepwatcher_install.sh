...
# edit the wakeup and sleep scripts appropiately
touch tmp_wakeup
touch tmp_sleep

sudo echo "#!/bin/sh
 curl -X POST  -H \"Accept: Application/json\" -H \"Content-Type: application/json\" http://yoururl.com/StatusChange.php -d '{\"status\":\"waking up\"}'
" > tmp_wakeup
sudo echo "#!/bin/sh
 curl -X POST  -H \"Accept: Application/json\" -H \"Content-Type: application/json\" http://yoururl.com/StatusChange.php -d '{\"status\":\"going to sleep\"}'
" > tmp_sleep

sudo cp tmp_wakeup /etc/rc.wakeup
sudo cp tmp_sleep /etc/rc.sleep
